...
int test()
{
	...
	myimage = cv::imread(image_path, CV_LOAD_IMAGE_COLOR);
	if (!myimage.data)
	{
		std::cout << "Fail to open image\n";
		return -1;
	}
	std::cout << "src image: cols = " << myimage.cols << ", rows = " << myimage.rows <<"\n";
	...
	Mat patch(height, width, myimage.type(), Scalar(0, 0, 255));
	std::cout << "width = " << patch.cols << "\n";
	std::cout << "height = " << patch.rows << "\n";
	getTexture(patch, myimage, homo);
